...
async(req:Request , res:Response)=>{
    try {
        let teacherRepo = await getRepository(User)
    ...
    let a = await teacherRepo.findOne({
        where:{
    ...
    if(a){
      ...
      newTeacher.password = secPassword;
      newTeacher.username = userName
      await teacherRepo.save(newTeacher)
      signJWT(newTeacher,(error , token)=>{
          if(error){
            res.send("unable to generate token");
          }
          else if(token){
            res.status(201).json({token : token , message:"user created" , newTeacher:newTeacher});
          }
         })
    }
...
router.post('/login' ,[
  ...
  body('password').notEmpty().withMessage('Password is required'),
    ...
    try {
      ...
      //do user exist?
      let user = await teacherRepo.findOne({
        where:{
          email: req.body.email
        }
      })
...
router.get("/getUser" ,extractJWT ,async(req:Request , res:Response)=>{
  ...
  res.status(201).send({teacherFound})
})
...
router.delete('/deleteUser' , extractJWT , async(req:Request , res:Response)=>{
  try {
    ...
    if(teacherFound){
      let a = await teacherRepo.delete(teacherFound.id)
